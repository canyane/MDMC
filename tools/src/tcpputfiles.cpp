/*
 *  程序名：tcpputfiles.cpp，此程序采用tcp协议，实现文件上传的客户端
 *  作者：CarlYim
 */
...
// 如果调用_tcpputfiles发送了文件，bcontinue为true，初始化为true。
bool bcontinue = true;
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        _help();
        /*
        ./tcpputfiles ../../idc/log/tcpputfiles_surfdata.log 
        "<ip>192.168.150.5</ip><port>5005</port><ptype>1</ptype>
        <clientpath>/home/wydxry/demo/idc/tmp/client</clientpath>
        <clientpathbak>/home/wydxry/demo/idc/tmp/backup</clientpathbak>
        <andchild>true</andchild><matchname>*.XML,*.CSV</matchname>
        <srvpath>/home/wydxry/demo/idc/tmp/server</srvpath><timetvl>10</timetvl>
...
{
    printf("\n");
    printf("Using:/demo/tools/bin/tcpputfiles logfilename xmlbuffer\n\n");

    printf("Sample:/demo/tools/bin/procctl 20 /demo/tools/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>192.168.174.133</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
    printf("       /demo/tools/bin/procctl 20 /demo/tools/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>192.168.174.132</ip><port>5005</port><ptype>2</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n\n\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
    ...
    starg.timetvl = min(starg.timetvl, 30);
...
bool SendFile(const int sockfd, const char *filename, const int filesize)
{
    ...
    while (true)
    {
        ...
        onread = min(filesize - totalbytes, 1000);
