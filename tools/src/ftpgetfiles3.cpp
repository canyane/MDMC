/*
 *  程序名：ftpgetfiles3.cpp，本程序是通用的功能模块，用于把远程ftp服务器的文件下载到本地目录
 *  作者：CarlYim
*/
...
bool _ftpgetfiles() 
{
    ...
    for (int i = 0; i < vlistfile.size(); i++) {
        ...
        if (starg.ptype == 3) {
            ...
            SNPRINTF(strremotefilenamebak, sizeof(strremotefilenamebak), 300, "%s/%s", starg.remotepathbak, vlistfile[i].filename);
