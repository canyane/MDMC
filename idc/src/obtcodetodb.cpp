...
int main(int argc, char const *argv[])
{
    ...
    if (argc != 5) {
        ...
        printf("Example:/project/tools/bin/procctl 120 /project/idc/bin/obtcodetodb /project/idc/ini/stcode.ini \"127.0.0.1,root,testmysql,mysql,3306\" utf8 /log/idc/obtcodetodb.log\n\n");
    ...
    if (LoadSite(argv[1]) == false) return -1;
...
bool LoadSite(const char *inifile) {
    ...
    while (true) {
        ...
        memset(&site, 0, sizeof(struct st_stcode));
        CmdStr.GetValue(0, site.provname, 30); // 省份
        CmdStr.GetValue(1, site.obtid, 10); // 站号
        CmdStr.GetValue(2, site.cityname, 30); // 站名
        CmdStr.GetValue(3, site.lat, 10); // 纬度
        CmdStr.GetValue(4, site.lon, 10); // 经度
        CmdStr.GetValue(5, site.height, 10); // 海拔高度
        
        // 把站点参数结构体放入站点参数容器
        vstcode.push_back(site);
    }

    // 查看是否载入成功
    for (int i = 0; i < vstcode.size(); i++) {
        logfile.Write("provname = %s, obtid = %s, cityname = %s, lat = %.2f, lon = %.2f, height = %.2f\n", \
                    vstcode[i].provname, vstcode[i].obtid, vstcode[i].cityname, vstcode[i].lat, vstcode[i].lon, vstcode[i].height);
    }
